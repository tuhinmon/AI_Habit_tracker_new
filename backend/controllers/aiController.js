...
export const suggesHabits = async (req, res) => {
      try{
              ...
              suggestion = [
                 {
                     ...
                     category : "Fitness",
                     icon : "",
                     reason: "Low-friction way to bulid consistency early in the day",
                 },
                 {
                     name : "Read 5 pages",
                     description : "Short daily reading to bulid a learning routine",
                     frequency : "daily",
                     category : "Learning",
                     icon : "",
...
const userMsg = `Habit: ${habit.name} (${habit.category}).\nDescription: ${habit.description || "none"}.\nCurrent streak : ${current} days. Longest ever: ${longest} days. The user just broke a streak. Write a warm, actioable 3-day recovery plan.`;
...
const context = habits
 .map((h) => {
  ...
  const byDow = [0, 0, 0, 0, 0, 0, 0];
   ...
   return `${h.name} (${h.category}); ${hLogs.length}/30 in last 30 days, by weekday [sun, mon , Tue, Wed, Thus, Fri, Sat]  = ${JSON.stringify(byDow)}`;
  })
.join("\n");
...
export const morningMotivation = async (req, res) => {
     try{
        ...
        if(!habits.length) {
            return res.json({
                content : 
                "Good morning! Add your first habit today and let's get the momentm started",
            });
        }
        const days = lastNDays(30);
        const logs = await HabitLog.find({
        ...
        const userMsg = `Today's habits and streaks:\n${ctx}\n\mDone today: ${done}/${total}. Write the morning message now.`
