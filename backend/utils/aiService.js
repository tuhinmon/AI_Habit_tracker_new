...
export const chatCompletion  = async ({ system, user, temperature = 0.7 }) => {
   ...
   if (!c) {
    return {
        ok: false,
        content:
        "AI features are disabled set GEMINI_API KEY in the backend env to enable real AI responses."
       };
    }
  try {
 ...
 return {ok: true, content: (res.text || "").trim() };
   ...
   console.errог ("АІ еггог:", err.message);
...
export const SYSTEM_PROMPTS = {
     ...
     suggestion:
      "You are a helpful habit coach. Based on the user's goals, productive time, and past struggles,suggest exactly 3 personalised habits. Return valid JSON only with this shape: {\"suggestion\":[{\"name\":\"...\",\"description\":\"...\",'frequency\":\"daily|weekly\",\"categoty\":\Health|Fithness|Learning|Mindfulness|Productivity|Social|Finance|Creative|Other\",\"icon\":\"<emoji>\",\"reason\":\"...\"}]}.No prose outside JSON",
      recovery:
      "You are a compassionate habit recovery coach. The user broke a streak. Write a 3 day recovery plan tailored to this specific habit. Be warm but actionable. Use this structure: short empathetic opening (1-2 sentences), then Day 1 / Day 2 / Day 3 sections with one concrete action each, them a closing line of eccouragement, 150-220 words total.",
      chat:
